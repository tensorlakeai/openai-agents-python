...
# Public-cloud exposed-port host. Only correct for the default Tensorlake control plane;
# a custom `proxy_url`/`api_url` deployment routes exposed ports through its own domain.
...
# The SDK reaches such sandboxes by sending requests to the proxy base URL with a
# `Host: {sandbox}.local` header, so there is no routable `host:port` URL we can hand an
# external HTTP client to reach an exposed port.
_LOOPBACK_HOSTS = frozenset({"localhost", "127.0.0.1", "::1"})


@dataclass(frozen=True)
class _ProxyHostResolution:
    """Outcome of resolving the exposed-port proxy host from `info().sandbox_url`.

    The three cases are mutually exclusive and drive distinct exposed-port behavior:

    - ``hostname`` set: a routable host (public cloud or custom remote deployment) that
      exposed ports reach via the ``{port}-{hostname}`` subdomain.
    - ``is_loopback``: the proxy is local/self-hosted; routing is via a ``Host`` header,
      not a subdomain, so no usable URL exists for an external client.
    - neither: ``sandbox_url`` could not be read. For the public control plane this is fine
      (the public template applies); for a custom control plane it is a retry-able miss.

    ``final`` marks a definitive answer that may be cached; a retry-able miss is not cached
    so a later lookup re-probes the backend.
    """
...
class TensorlakeSandboxClientOptions(BaseSandboxClientOptions):
    ...
    Attributes:
        ...
        proxy_url: Optional override for the Tensorlake sandbox proxy URL (e.g., for
            ...
            from `AsyncSandbox.info().sandbox_url` instead of the public template. A local
            proxy (one whose `sandbox_url` resolves to localhost) routes via a `Host`
            header rather than a subdomain, so it has no URL form; `resolve_exposed_port`
            raises `ExposedPortUnavailableError` rather than returning an unroutable URL.
...
class TensorlakeSandboxSession(BaseSandboxSession):
    ...
    _proxy_host_resolution: _ProxyHostResolution | None
    ...
    def __init__(
        ...
        *,
        ...
        self._proxy_host_resolution = None
    ...
    def _custom_control_plane(self) -> bool:
        """Whether a non-default proxy/control-plane is in effect.

        A custom deployment routes exposed ports through its own domain, so the public
        ``*.sandbox.tensorlake.ai`` template never applies to it.
        """
        return self.state.proxy_url is not None or self.state.api_url is not None

    async def _resolve_exposed_port(self, port: int) -> ExposedPortEndpoint:
        # Prefer the backend's per-sandbox URL so non-default proxy/control-plane
        # deployments (e.g. tensorlake.dev) resolve correctly; fall back to the public
        # template only for the default public control plane.
        resolution = await self._resolve_proxy_host()

        if resolution.hostname is not None:
            # Routable host (public cloud or custom remote deployment): exposed ports use
            # port-prefixed subdomain routing.
            return ExposedPortEndpoint(host=f"{port}-{resolution.hostname}", port=443, tls=True)

        if resolution.is_loopback:
            # Local/self-hosted proxy. The SDK reaches the sandbox by sending requests to the
            # proxy base URL with a `Host` header rather than a routable subdomain, so there is
            # no plain URL an external client can use to reach the exposed port. Returning the
            # public `*.sandbox.tensorlake.ai` template here would hand back a URL that points
            # at the public cloud, not the local proxy, so surface an explicit error instead.
            raise ExposedPortUnavailableError(
                port=port,
                exposed_ports=self.state.exposed_ports,
                reason="local_proxy_no_public_url",
                context={
                    "backend": "tensorlake",
                    "sandbox_id": self.state.sandbox_id,
                    "proxy_url": self.state.proxy_url,
                    "api_url": self.state.api_url,
                    "sandbox_url_host": resolution.raw_host,
                },
            )

        if self._custom_control_plane():
            # Custom control plane whose `sandbox_url` could not be read (likely a transient
            # info()/status() failure). The public template cannot route to a custom
            # deployment, so raise a retry-able error rather than return a misleading URL.
            raise ExposedPortUnavailableError(
                port=port,
                exposed_ports=self.state.exposed_ports,
                reason="sandbox_url_unresolved",
                context={
                    "backend": "tensorlake",
                    "sandbox_id": self.state.sandbox_id,
                    "proxy_url": self.state.proxy_url,
        ...
        # Default public control plane: the public per-sandbox template is correct.
    ...
    async def _resolve_proxy_host(self) -> _ProxyHostResolution:
        cached = self._proxy_host_resolution
        if cached is not None and cached.final:
            return cached

        custom_control_plane = self._custom_control_plane()
        sandbox_url = await self._probe_sandbox_url(custom_control_plane=custom_control_plane)
        raw_host = urlsplit(sandbox_url).hostname if sandbox_url else None

        if raw_host:
            if raw_host in _LOOPBACK_HOSTS:
                # Definitive: a local proxy stays local for the life of this backend handle.
                resolution = _ProxyHostResolution(
                    hostname=None, is_loopback=True, final=True, raw_host=raw_host
                )
            else:
                resolution = _ProxyHostResolution(
                    hostname=raw_host, is_loopback=False, final=True, raw_host=raw_host
                )
        elif custom_control_plane:
            # Could not read `sandbox_url`. Treat as a retry-able miss (do not cache) so a
            # later lookup re-probes once the backend recovers; `_resolve_exposed_port`
            # raises rather than returning the inapplicable public template.
            resolution = _ProxyHostResolution(
                hostname=None, is_loopback=False, final=False, raw_host=None
            )
        else:
            # Default public control plane: the public template applies, so this is final.
            resolution = _ProxyHostResolution(
                hostname=None, is_loopback=False, final=True, raw_host=None
            )

        self._proxy_host_resolution = resolution
        return resolution

    async def _probe_sandbox_url(self, *, custom_control_plane: bool) -> str | None:
        try:
        ...
        if custom_control_plane and not sandbox_url:
            ...
            with suppress(Exception):
                ...
                info = await self._sandbox.info()
                sandbox_url = getattr(info, "sandbox_url", None) if info is not None else None
        return sandbox_url if isinstance(sandbox_url, str) and sandbox_url else None
    ...
    async def _restore_from_checkpoint(self, snapshot_id: str) -> None:
        ...
        if new_id is None:
            ...
            raise WorkspaceArchiveWriteError(
                path=error_root,
                context={
                    "reason": "tensorlake_checkpoint_restore_missing_sandbox_id",
                    "snapshot_id": snapshot_id,
                },
            )
...
class TensorlakeSandboxClient(BaseSandboxClient[TensorlakeSandboxClientOptions]):
    ...
    async def resume(
        ...
        sandbox: Any = None
        reconnected = False
        ambiguous_paused_reconnect_error: BaseException | None = None
        try:
            ...
            if state.pause_on_exit:
                ...
                # what actually transitions it to running.
            ...
            if state.pause_on_exit:
                # The user opted into the suspend lifecycle and expects the backend to
                # preserve workspace state across resume, so treat ANY failure as
                # ambiguous — whether `connect()` itself failed or a post-connect probe
                # (`resume`/`status`) did. `AsyncSandbox.connect()` only wires up a proxy
                # client and returns a handle without verifying the sandbox exists
                # (existence is probed later by `resume()`/`status()`), so a `connect()`
                # failure is a transport blip, not proof the named sandbox is gone. The
                # named sandbox may still be suspended on the backend, so recreating below
                # with the same preserved name would either hit a name conflict or move
                # the session off the preserved in-place state (silently restoring an
                # empty/Noop or stale snapshot). Surface it instead and let the caller
                # retry; the remote reconnects later or auto-expires via `timeout_secs`.
                # Drop any local handle without terminating, to keep recoverable state.
                if sandbox is not None:
                    with suppress(Exception):
                        sandbox.close()
                ambiguous_paused_reconnect_error = exc
            elif sandbox is not None:
                # Without the suspend lifecycle, no cross-resume state is expected.
        ...
        if ambiguous_paused_reconnect_error is not None:
            sandbox_name = _resolve_lifecycle_sandbox_name(
                name=state.name,
            ...
            raise WorkspaceStartError(
                path=Path(state.manifest.root),
                message="failed to resume preserved Tensorlake sandbox",
                context={
                    "reason": "tensorlake_paused_reconnect_ambiguous",
                    "sandbox_id": state.sandbox_id,
                    "name": sandbox_name,
                },
                cause=ambiguous_paused_reconnect_error,
            ) from ambiguous_paused_reconnect_error
        ...
        if sandbox is None:
            ...
            if new_id is None:
                ...
                raise WorkspaceStartError(
                    path=Path(state.manifest.root),
                    message="failed to recreate Tensorlake sandbox",
                    context={
                        "reason": "tensorlake_recreate_missing_sandbox_id",
                        "sandbox_id": state.sandbox_id,
                        "snapshot_id": recreate_snapshot_id,
                    },
                )
