...
@pytest.mark.asyncio
async def test_resume_recreate_rejects_created_sandbox_without_resolved_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agents.sandbox.errors import WorkspaceStartError
    ...
    async def _missing_sandbox_id(_sandbox: object) -> str | None:
        return None

    monkeypatch.setattr(module, "_resolve_sandbox_id", _missing_sandbox_id)

    state = module.TensorlakeSandboxSessionState(
        session_id=uuid.UUID("00000000-0000-0000-0000-000000000064"),
        manifest=Manifest(),
        snapshot=NoopSnapshot(id="snap"),
        sandbox_id="sandbox-dead",
        pause_on_exit=False,
    ...
    assert exc_info.value.context["reason"] == "tensorlake_recreate_missing_sandbox_id"
    assert exc_info.value.context["sandbox_id"] == "sandbox-dead"
    recreated = _FakeSandbox.sandboxes["tensorlake-sandbox-1"]
    assert recreated.terminated is True
    assert state.sandbox_id == "sandbox-dead"
...
async def test_custom_proxy_exposed_port_resolution_retries_after_transient_failure(
    ...
    fake.info = flaky_info  # type: ignore[method-assign]
    fake.status = flaky_status  # type: ignore[method-assign]

    session = module.TensorlakeSandboxSession.from_state(state, sandbox=fake)

    from agents.sandbox.errors import ExposedPortUnavailableError

    # First lookup hits the transient failures. The public template cannot route to a
    # custom-control-plane deployment, so this raises a retry-able error rather than
    # returning a misleading public URL, and the miss must NOT be cached.
    with pytest.raises(ExposedPortUnavailableError) as first_exc:
        await session.resolve_exposed_port(8080)
    assert first_exc.value.context["reason"] == "sandbox_url_unresolved"
...
async def test_local_proxy_exposed_port_resolution_raises_instead_of_public_url(
    monkeypatch: pytest.MonkeyPatch,
    sandbox_url: str,
) -> None:
    module = _load_tensorlake_module(monkeypatch)
    state = module.TensorlakeSandboxSessionState(
        session_id=uuid.UUID("00000000-0000-0000-0000-00000000005a"),
        manifest=Manifest(),
        snapshot=NoopSnapshot(id="snap"),
    ...
    # A local proxy routes via a `Host` header, not a subdomain, so there is no URL form.
    # We must raise rather than hand back the public `*.sandbox.tensorlake.ai` template,
    # which points at the public cloud and cannot reach the local deployment.
    with pytest.raises(ExposedPortUnavailableError) as exc_info:
        await session.resolve_exposed_port(8080)
    assert exc_info.value.context["reason"] == "local_proxy_no_public_url"
    assert "tensorlake.ai" not in str(exc_info.value)

    # The loopback answer is definitive, so it is cached: a second lookup must not re-probe.
...
@pytest.mark.asyncio
async def test_resume_preserves_paused_sandbox_when_resume_probe_fails(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A failed paused-sandbox probe must leave the named backend available for retry."""

    module = _load_tensorlake_module(monkeypatch)

    cause = RuntimeError("transient resume failure")
    existing = _FakeSandbox(sandbox_id="sandbox-paused", status="suspended")
    existing.resume_failure = cause
    ...
    assert exc_info.value.__cause__ is cause
    assert exc_info.value.message == "failed to resume preserved Tensorlake sandbox"
    assert exc_info.value.context["reason"] == "tensorlake_paused_reconnect_ambiguous"
    assert exc_info.value.context["sandbox_id"] == "sandbox-paused"
    assert exc_info.value.context["name"] == "openai-agents-0000000000000000000000000000000f"
    assert _FakeSandbox.create_calls == []
    assert state.sandbox_id == "sandbox-paused"
    assert state.workspace_root_ready is False
...
@pytest.mark.asyncio
async def test_resume_preserves_paused_sandbox_when_connect_fails(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A `connect()` failure under pause_on_exit must not recreate the named sandbox.

    `AsyncSandbox.connect()` returns a handle without verifying the sandbox exists, so a
    `connect()` failure is a transport blip rather than proof the sandbox is gone. The
    named sandbox may still be suspended, so recreating with the same preserved name
    could hit a name conflict or silently replace the preserved in-place state. The
    resume must surface an ambiguous error and leave the backend untouched for retry.
    """

    module = _load_tensorlake_module(monkeypatch)

    cause = RuntimeError("transient connect failure")
    _FakeSandbox.connect_failures["sandbox-missing"] = cause
    ...
    assert exc_info.value.__cause__ is cause
    assert exc_info.value.context["reason"] == "tensorlake_paused_reconnect_ambiguous"
    assert exc_info.value.context["sandbox_id"] == "sandbox-missing"
    assert exc_info.value.context["name"] == "openai-agents-0000000000000000000000000000000f"
    assert _FakeSandbox.create_calls == []
    assert state.sandbox_id == "sandbox-missing"
    assert state.workspace_root_ready is False


@pytest.mark.asyncio
async def test_resume_preserves_paused_sandbox_when_connect_fails_in_snapshot_mode(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """A `connect()` failure under pause_on_exit + snapshot mode must not recreate.

    Recreating here would terminate-and-restore from the stored snapshot, silently
    moving the session off the still-suspended in-place state and onto a potentially
    stale snapshot. The resume must surface an ambiguous error instead.
    """
    ...
    cause = RuntimeError("transient connect failure")
    _FakeSandbox.connect_failures["sandbox-paused"] = cause

    state = module.TensorlakeSandboxSessionState(
        session_id=uuid.UUID("00000000-0000-0000-0000-000000000012"),
        manifest=Manifest(),
        snapshot=snapshot,
        sandbox_id="sandbox-paused",
        pause_on_exit=True,
...
@pytest.mark.asyncio
async def test_resume_preserves_paused_sandbox_when_snapshot_resume_probe_fails(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Snapshot-mode resume must not recreate after an ambiguous named reconnect failure."""
...
@pytest.mark.asyncio
async def test_restore_from_checkpoint_rejects_created_sandbox_without_resolved_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from agents.sandbox.errors import WorkspaceArchiveWriteError
    ...
    state = module.TensorlakeSandboxSessionState(
        session_id=uuid.UUID("00000000-0000-0000-0000-000000000065"),
        manifest=Manifest(),
        snapshot=NoopSnapshot(id="snap"),
        sandbox_id="sandbox-before-restore",
        workspace_persistence="snapshot",
    )
    old_sandbox = _FakeSandbox(sandbox_id="sandbox-before-restore")
    session = module.TensorlakeSandboxSession.from_state(state, sandbox=old_sandbox)

    payload = module._encode_tensorlake_snapshot_ref(snapshot_id="snap-missing-id")
    with pytest.raises(WorkspaceArchiveWriteError) as exc_info:
        await session.hydrate_workspace(io.BytesIO(payload))

    assert exc_info.value.context["reason"] == "tensorlake_checkpoint_restore_missing_sandbox_id"
    assert exc_info.value.context["snapshot_id"] == "snap-missing-id"
    recreated = _FakeSandbox.sandboxes["tensorlake-sandbox-1"]
    assert recreated.terminated is True
    assert session._sandbox is old_sandbox
    assert state.sandbox_id == "sandbox-before-restore"
...
async def test_resume_recreate_from_memory_checkpoint_strips_snapshot_owned_kwargs(
    ...
    `client.resume(state)` falls back to `AsyncSandbox.create(snapshot_id=...)` when
    `AsyncSandbox.connect(...)` cannot find the old sandbox. That second create site has to
    apply the same memory-snapshot filtering as `_restore_from_checkpoint`, otherwise any
    non-default user config breaks the recreate flow.

    Uses `pause_on_exit=False` so the connect-miss recreate path runs: under the suspend
    lifecycle a `connect()` failure is treated as ambiguous and surfaced instead of
    recreating (see `test_resume_preserves_paused_sandbox_when_connect_fails`).
    """
    module = _load_tensorlake_module(monkeypatch)
    ...
    state = module.TensorlakeSandboxSessionState(
        ...
        manifest=Manifest(),
        snapshot=snapshot,
        sandbox_id="sandbox-missing-mem",
        image="custom-image",
        cpus=2.0,
        memory_mb=2048,
